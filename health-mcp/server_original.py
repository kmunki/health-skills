...
def check_duplicate_narrative(user_id: str, narrative: str, threshold: float = 0.85) -> Optional[Dict]:
    """
    Check if a similar narrative already exists for this user.
    ...
    Returns:
        The most similar existing event if found, None otherwise
    """
    ...
    try:
        ...
        # Check similarity with existing narratives
        best_match = None
        best_score = 0
        
        for event in events:
            existing_narrative = event.get('narrative', '')
            score = SequenceMatcher(None, narrative.lower(), existing_narrative.lower()).ratio()
...
def record_health_narrative(
    ...
    Args:
        ...
        auto_extract: Whether to automatically extract structured data (default: False)
    """
    try:
        # Check for duplicate narrative
        duplicate = check_duplicate_narrative(user_id, narrative)
        if duplicate:
            return (f"Duplicate narrative detected! Similar event already exists "
                   f"(ID: {duplicate['id']}, Date: {duplicate['date']}). "
                   f"Consider updating the existing event instead.")
        ...
        result = f"Recorded health narrative (ID: {event['id']})"
        
        # Auto-extract if requested
        if auto_extract:
            try:
                structured_json = extract_health_info(narrative)
                structured_data = json.loads(structured_json)
                if structured_data:  # Only update if extraction found something
                    update_result = update_event_structure(event['id'], user_id, structured_data)
                    result += f"\n{update_result}"
            except Exception as e:
                result += f"\nAuto-extraction failed: {str(e)}"
...
def update_event_structure(
    ...
    Args:
        event_id: ID of the event to update (accepts partial IDs - first 8 chars minimum)
        user_id: User identifier
    ...
    try:
        ...
        # Find and update the event (supports partial ID matching)
        updated = False
        for event in events:
            # Check for exact match or partial match (minimum 8 chars)
            if (event['id'] == event_id or 
                (len(event_id) >= 8 and event['id'].startswith(event_id))):
        ...
        return f"Updated structured data for event: {event_id}"
    except Exception as e:
